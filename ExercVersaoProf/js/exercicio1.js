window.addEventListener('load', function () {
	...
	for (p = 0; p < produtos.length; p++) {
		...
		eLink.addEventListener('click', function () {
			...
			requisicao('GET', 'http://localhost:8080/jsav/compra.php', 'id=' + this.getAttribute('id'), function () {
				if (HTTPRequest.readyState === HTTPRequest.DONE) {
					if (HTTPRequest.status === 200) {
					}
				}
			...
			requisicao('GET', 'http://localhost:8080/jsav/consultaTotal.php', null, fn, false);
		});
